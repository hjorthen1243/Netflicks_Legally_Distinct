...
public class CategoryDAO implements ICategoryDAO {
    ...
    public List<Category> getMovieCategories() {
        ...
        try (Connection conn = databaseConnector.getConnection()) {
            ...
            ResultSet rs = stmt.executeQuery(sql);
            while (rs.next()) {
                int id = rs.getInt("id"); //Map the ID of the Category
                String catName = rs.getString("Category"); //Map the name of the Category
                Category category = new Category(id, catName); //Create and add Categories to list
    ...
    public List<Category> getUpdatedCategories(List<Category> categories) {
        ...
        try (Connection conn = databaseConnector.getConnection()) {
            ...
            while (rs.next()) {
                ...
                String catName = rs.getString("Category");
                Category category = new Category(id, catName);
                categories1.add(category); //The result is mapped and added to the new Array List of categories.
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
